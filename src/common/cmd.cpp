...
void Cbuf_AddText(const char* text)
{
	...
	SZ_Write(&cmd_text, (void*)text, (int32_t)strlen(text));
}
...
void Cbuf_InsertText(const char* text)
{
	...
	if (templen)
	{
		temp = (char*)Memory_ZoneMalloc(templen);
...
bool Cbuf_AddLateCommands()
{
	...
	text = (char*)Memory_ZoneMalloc(s + 1);
	text[0] = 0;
	for (i = 1; i < argc; i++)
	{
	...
	build = (char*)Memory_ZoneMalloc(s + 1);
	build[0] = 0;
...
void Cmd_Exec_f()
{
	...
	f2 = (char*)Memory_ZoneMalloc(len + 1);
	memcpy(f2, f, len);
	f2[len] = 0;
...
void Cmd_Alias_f()
{
	...
	if (!a)
	{
		a = (cmdalias_t*)Memory_ZoneMalloc(sizeof(cmdalias_t));
		a->next = cmd_alias;
		cmd_alias = a;
	}
	strcpy(a->name, s);
...
char* Cmd_MacroExpandString(const char* text)
{
	...
	inquote = false;
	scan = (char*)text; // stupid but whatever
...
============
*/
void Cmd_TokenizeString(const char* text, bool macro_expand)
{
	int32_t 	i;
...
void Cmd_AddCommand(const char* cmd_name, xcommand_t function)
{
	...
	cmd = (cmd_function_t*)Memory_ZoneMalloc(sizeof(cmd_function_t));
	cmd->name = cmd_name;
